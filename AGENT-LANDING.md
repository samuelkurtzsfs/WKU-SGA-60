...
## The pull request body is the other place attribution leaks

A commit you have cleaned is not the whole job. When a run opens a pull request,
a "Generated by Claude Code" line carrying a session link is appended to the
body, and on this repository that is the same violation as a `Co-Authored-By`
trailer: CLAUDE.md forbids session links in visible text, and a pull request
body is visible text published under the project's name. Older pull requests

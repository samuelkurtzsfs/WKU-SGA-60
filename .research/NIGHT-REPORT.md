...
The review comment posted on #313 carries a trailing "Generated by Claude Code"
line, which CLAUDE.md and AGENT-LANDING.md both forbid on anything published
under the project's name. It could not be removed from this run: the GitHub
tools available here can post a comment but cannot edit or delete one, and the
REST path that would has the platform gate on it. The comment is
...
# 6 September 2026, evening — no queue, so the advance-notice rule was tested against the whole archive

No pull request was open. #367 merged this afternoon and nothing has been pushed
since; the eleven research routines are still paused, unchanged from this
morning's check, so there was no research to review. The three pull requests
this pass is standing instructions to rescue — #6, #7 and #8 — remain closed
since 18 August, and that instruction is still out of date.
...
pairs it has reported for days, every one of them genuinely separate business.

## What was audited instead, and why this test

The one trap in this project that cannot be caught by reading an entry on its
own is the advance notice. "Mini-Concert Set Thursday" and a report of how the
night went look identical in isolation; what gives the first away is that the
event is dated *after* the issue that sources it, because a paper cannot report
a thing that has not happened yet. That is a comparison, not a judgement, so it
can be run over the whole archive at once.

757 events cite a *Herald* record whose publication date the local index knows.
**Twenty-six of them are dated after the issue they cite** — the advance-notice
signature, every one of them.

Twenty-three of the twenty-six turned out to be written correctly, and that is
worth recording as plainly as the failures. They say "the Herald announced",
"billed", "was planning", "ASG scheduled"; where they state that the night
actually happened they name the corroboration in the prose — the *Talisman*, or
the following week's review. The programme verification that ran earlier in this
project did its work, and this pass changed none of them.

A second sweep compared every citation label's own date against the index:
**two disagreements in the whole archive**, one of them explicable (a label
citing minutes and an issue together). The citation hygiene here is good.

## The three that did not hold, all rescued rather than cut

**1991-92, the fall elections.** The entry read: the Herald covered fall
elections on 10 October and a separate freshman election on 15 October, and
"the paper's abstracts do not preserve the results". Both halves were wrong. The
10 October item is *"Associated Student Government Elections Tuesday"* — an
announcement of the election on the coming Tuesday, which was 15 October, not
coverage of an election that day; the entry had turned one election into two.
And the results are preserved: the issue of 17 October carries **"Kevin Colon
Wins Freshman Election"** (`dlsc_ua_records/7787`), read on the record page
itself. This is exactly what CLAUDE.md warns about — a negative written out of a
miss in the local index, which is truncated and proves nothing. Rewritten to
what the three issues prove, with the result and its source added.

**1969-70, Congress endorses Downing's reply.** Dated 16 May 1970 and cited to
"Herald 49:49, 16 May 1970". The record is dated **5-15-1970**, and it carries
Paul Hightower's report of the unanimous vote. A vote cannot postdate the issue
reporting it: the event is 15 May, and the label was a day out. Both corrected,
and the reporter named.

**1972-73, the Bluegrass Festival.** The entry stated that ASG closed its season
on 29 April with the festival, sourced only to the announcement of 27 April —
an occurrence asserted from a notice. The report exists: Al Cross,
*"Bluegrass Festival—A Good Time for All"*, Herald 52:55 of 1 May 1973
(`dlsc_ua_records/5013`). Added as a second source, so the claim now rests on
the report rather than the booking. Nothing was cut.

Every source above was opened on digitalcommons and read, one request at a time,
three seconds apart. No 403s.

## The attribution footer, measured

The last pass recorded that the review comment on #367 carries a "Generated by
Claude Code" footer and wanted it deleting by hand. That was true but far too
small a number. A search of the repository returns **154 pull request bodies**
carrying the same line, and the platform appends it to comments as they are
posted, so it is not a stray that someone forgot to strip — it is being added
automatically and it will be added to the next one too. No tool available here
can edit or delete an existing comment. This belongs in the log as a standing
condition of the pull request layer rather than a chore: nothing in `data/` or
in the generated site carries it, which is what CLAUDE.md's rule protects, and
...
The eleven paused routines, unchanged and not an editor's call to reverse.
Mary Fyfe, from the last pass, still unlooked-at. And the twenty-three correctly
written advance-notice entries are worth a look by a future pass for a different
